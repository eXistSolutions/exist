...
function syse:defaults() {
    ...
    let $sync := helper:sync-with-options($directory, ())
...
function syse:indent-no() {
    let $directory := helper:get-test-directory($syse:suite)
    let $sync := helper:sync-with-options($directory, map{"indent": false()})
...
function syse:indent-yes() {
    let $directory := helper:get-test-directory($syse:suite)
    let $sync := helper:sync-with-options($directory, map{"indent": true()})
...
function syse:omit-xml-declaration-no() {
    let $directory := helper:get-test-directory($syse:suite)
    let $sync := helper:sync-with-options($directory, map{"omit-xml-declaration": false()})
...
function syse:omit-xml-declaration-yes() {
    let $directory := helper:get-test-directory($syse:suite)
    let $sync := helper:sync-with-options($directory, map{"omit-xml-declaration": true()})
